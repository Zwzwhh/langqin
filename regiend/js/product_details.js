...
(()=>{	
	...
	$.ajax({
		...
		dataType:"json"
			...
			var {wid,title,wname,price,wglass,size,proof,mater,shape
			...
			document.querySelector("#xqmoreContent")
			.innerHTML=`<div class="img-content" >
				<img src="${sm}" alt="" width=100%>
			</div>
			<div class="text-content">
				<a href="">${title}</a>
				<div>材质:${mater}</div>
				<div>表盘和指针:${needle}</div>
				<div>表带:${cmater}</div>
				<p>¥${price}</p>
			</div>`;
